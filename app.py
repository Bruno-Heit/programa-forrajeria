...
class MainWindow(QMainWindow):
    def __init__(self, db_path: str = DATABASE_DIR):
        ...
        self.config: SettingsManager = SettingsManager(
            app_name=PV.APP_NAME.value, organization_name=PV.APP_AUTHOR.value
        )
    ...
    def setup_ui(self) -> None:
        ...
        self.__setInitDateWidgets(
            widget=self.ui.dateEdit_from_date, date=getWeekStartDate(_today)
        )
        self.__setInitDateWidgets(widget=self.ui.dateEdit_to_date, date=_today)
        self.__setInitDateWidgets(
            widget=self.ui.dateEdit_show_collected_by_day, date=_today
        )
    ...
    def install_event_filters(self) -> None:
        ...
        """
        # categorías
        self.category_list_event_filter = CategoryListEventFilter(
            self.ui.tables_ListWidget
        )
        ...
        self.ui.tv_inventory_data.viewport().installEventFilter(
            self.inv_table_event_filter
        )

        # ventas
        self.sales_input_list_event_filter = BackgroundEventFilter(
            self.ui.sales_input_list
        )
        self.ui.sales_input_list.viewport().installEventFilter(
            self.sales_input_list_event_filter
        )

        self.sales_table_event_filter = BackgroundEventFilter(self.ui.tv_sales_data)
        self.ui.tv_sales_data.viewport().installEventFilter(
            self.sales_table_event_filter
        )

        # deudas
        self.debts_table_event_filter = BackgroundEventFilter(self.ui.tv_debts_data)
        self.ui.tv_debts_data.viewport().installEventFilter(
            self.debts_table_event_filter
        )
        return None
    ...
    def setup_models(self) -> None:
        ...
        self.inventory_proxy_model.setFilterCaseSensitivity(
        ...
        self.sales_proxy_model.setFilterCaseSensitivity(
            Qt.CaseSensitivity.CaseInsensitive
        )
        ...
        self.debts_proxy_model.setFilterCaseSensitivity(
            Qt.CaseSensitivity.CaseInsensitive
        )
    ...
    def setup_inventory_signals(self) -> None:
        ...
        self.ui.btn_side_barToggle.clicked.connect(
            lambda: self.toggleSideBar(TypeSideBar.CATEGORIES_SIDEBAR)
        )

        self.ui.btn_inventory_sideBarToggle.clicked.connect(
            lambda: self.toggleSideBar(TypeSideBar.PERCENTAGES_SIDEBAR)
        )

        # * (READ) cargar con productos 'tv_inventory_data'
        self.ui.tables_ListWidget.itemDoubleClicked.connect(
            lambda item: self.fillTableView(
                table_viewID=TableViewId.INVEN_TABLE_VIEW,
                ACCESSED_BY_LIST=True,
                SHOW_ALL=True
                if item.text() == CommonCategories.SHOW_ALL.value
                else False,
            )
        )
        ...
        # * delegado de inventario
        self.inventory_delegate.fieldIsValid.connect(
            self.__onDelegateValidationSucceded
        )
        self.inventory_delegate.fieldIsInvalid.connect(
            self.__onDelegateValidationFailed
        )

        # * inventory_sideBar
        self.ui.inventory_checkbuttons_buttonGroup.buttonPressed.connect(
            self.handlePressedCheckbox
        )
        self.ui.inventory_checkbuttons_buttonGroup.buttonClicked.connect(
            self.handleClickedCheckbox
        )

        self.ui.checkbox_unit_prices.stateChanged.connect(
            self.handleCheckboxStateChange
        )

        self.ui.checkbox_comercial_prices.stateChanged.connect(
            self.handleCheckboxStateChange
        )

        # * lineedit de porcentajes
        self.ui.lineEdit_percentage_change.returnPressed.connect(
            self.onLePercentageEditingFinished
        )
        ...
        self.ui.cb_inventory_colsFilter.currentIndexChanged.connect(
            lambda index: self.changeFilterColumn(
                column=index, tableViewID=TableViewId.INVEN_TABLE_VIEW
            )
        )
        ...
        self.category_list_event_filter.nameAboutToChange.connect(
            self.setEditableCategoryName
        )
        self.category_list_event_filter.descAboutToChange.connect(
            self.showCategoryEditDialog
        )
        return None
    ...
    def setup_sales_signals(self) -> None:
        ...
        self.ui.dateEdit_from_date.dateChanged.connect(
            lambda date: self.validateDateRange(
                dateedit=self.ui.dateEdit_from_date, date=date
            )
        )
        self.ui.dateEdit_to_date.dateChanged.connect(
            lambda date: self.validateDateRange(
                dateedit=self.ui.dateEdit_to_date, date=date
            )
        )
        ...
        self.ui.tabWidget.currentChanged.connect(
            lambda index: self.ui.tab2_toolBox.setCurrentIndex(0)
            if index == 1
            else None
        )
        ...
        self.sales_data_model.dataToUpdate.connect(
            lambda params: self.__onSalesModelDataToUpdate(
                ...
                quantity_index=params["quantity_index"]
                if "quantity_index" in params
                else None,
            )
        )
        ...
        self.ui.cb_sales_colsFilter.currentIndexChanged.connect(
            lambda index: self.changeFilterColumn(
                column=index, tableViewID=TableViewId.SALES_TABLE_VIEW
            )
        )
    ...
    def setup_debts_signals(self) -> None:
        ...
        self.ui.tabWidget.currentChanged.connect(
            lambda curr_index: self.fillTableView(
                table_viewID=TableViewId.DEBTS_TABLE_VIEW, SHOW_ALL=True
            )
            if curr_index == 2
            else None
        )
        ...
        self.ui.btn_add_debtor.clicked.connect(
            lambda: self.handleTableCreateRow(table_viewID=TableViewId.DEBTS_TABLE_VIEW)
        )
        ...
        self.ui.btn_delete_debtor.clicked.connect(lambda: self.__removeDebtsModelRows())
        ...
        self.debts_delegate.balanceDialogFinished.connect(
            self.__updateViewOnDebtsBalanceChanged
        )
        ...
        self.ui.cb_debts_colsFilter.currentIndexChanged.connect(
            lambda index: self.changeFilterColumn(
                column=index, tableViewID=TableViewId.DEBTS_TABLE_VIEW
            )
        )
        return None
    ...
    def __setTablesListWidgetItems(self) -> None:
        ...
        self.ui.tables_ListWidget.item(
            self.ui.tables_ListWidget.count() - 1
        ).setToolTip(
            """ <html>
                    <head/>
                    <body>
    ...
    def __setInitialIconsToWidgets(self) -> None:
        ...
        self.delete_register_dark_icon = (
            QIcon()
        )  # eliminar registros (color alternativo oscuro)
        ...
        self.ui.sales_searchBar.addAction(
            self.search_bars_icon, QLineEdit.ActionPosition.LeadingPosition
        )
        self.ui.debts_searchBar.addAction(
            self.search_bars_icon, QLineEdit.ActionPosition.LeadingPosition
        )
    ...
    def __setInitialStylesheets(self) -> None:
        ...
        """
        # comboboxes
        self.ui.cb_inventory_colsFilter.setStyleSheet(
            WidgetStyle.DEF_COMBOBOX_FILTER_ICON.value
        )
        self.ui.cb_sales_colsFilter.setStyleSheet(
            WidgetStyle.DEF_COMBOBOX_FILTER_ICON.value
        )
        self.ui.cb_debts_colsFilter.setStyleSheet(
            WidgetStyle.DEF_COMBOBOX_FILTER_ICON.value
        )

        # dateedits
        self.ui.dateEdit_from_date.setStyleSheet(
            WidgetStyle.DEF_DATEEDIT_ARROW_ICON.value
        )
        self.ui.dateEdit_to_date.setStyleSheet(
            WidgetStyle.DEF_DATEEDIT_ARROW_ICON.value
        )
        self.ui.dateEdit_show_collected_by_day.setStyleSheet(
            WidgetStyle.DEF_DATEEDIT_ARROW_ICON.value
        )
    ...
    def __initGeneralValidators(self) -> None:
        ...
        """
        # validadores
        self.inventory_search_bar_validator = SearchBarValidator(
            self.ui.inventory_searchBar
        )
        ...
        self.total_paid_validator = SalePaidValidator(
            self.ui.lineEdit_paid, is_optional=True
        )
    ...
    def __createCategoryNameEditor(
        self, item: QListWidgetItem, edit_mode: bool = False
    ) -> QLineEdit:
        """
        ...
        **NOTA:** éste método conecta la señal *editingFinished* a una función
        ...
        validator = CategoryNameValidator(
            parent=editor, prev_name=item.text() if edit_mode else None
        )
        ...
        filter_event: CategoryItemEventFilter = CategoryItemEventFilter(
            lineedit=editor, item=item, edit_mode=edit_mode
        )
        editor.installEventFilter(filter_event)

        # conecta señales
        editor.editingFinished.connect(lambda: editor.setText(editor.text().strip()))
        ...
        validator.isEmpty.connect(lambda: filter_event.setValidity(validity=None))
        validator.isEmpty.connect(lambda: editor.setStyleSheet(""))
    ...
    def __categoryEditorOnConfirmedName(self, item: QListWidgetItem, name: str) -> None:
        ...
        if name:
            ...
            item.setText(name)
            self.ui.tables_ListWidget.setItemWidget(
                item, None
    ...
    def __makeCategoryListLastsConfigs(self) -> None:
        ...
        **MOSTRAR TODOS**.
        ...
        _item_show_all = self.ui.tables_ListWidget.findItems(
            CommonCategories.SHOW_ALL.value, Qt.MatchFlag.MatchExactly
        )
    ...
    def toggleCategoryDeleteButton(self, selected_items: list[QListWidgetItem]) -> None:
        ...
        if _selected_item.text().strip().upper() not in (
            CommonCategories.MISC.value.upper(),
    ...
    def setEditableCategoryName(self, item: QListWidgetItem) -> None:
        ...
        """
        # crea editor
        editor: QLineEdit = self.__createCategoryNameEditor(item=item, edit_mode=True)
    ...
    def showCategoryEditDialog(self, item: QListWidgetItem) -> None:
        ...
        """
        # crea dialog
        category_dialog = CategoryDescDialog(list_item=item)
    ...
    def __setNpDataAccumulator(
        self, table_viewID: TableViewId, model_shape: tuple[int, int]
    ) -> None:
        """
        ...
        match table_viewID.name:
            ...
            case "SALES_TABLE_VIEW":  # Ventas
                self._sales_model_data_acc = empty(shape=model_shape, dtype=object)

            case "DEBTS_TABLE_VIEW":  # Ctas. Ctes.
                self._debts_model_data_acc = empty(shape=model_shape, dtype=object)
        return None
    ...
    def __initSelectWorker(
        ...
        self.select_worker.countFinished.connect(
            lambda model_shape: self.__workerSelectOnCountFinished(
                table_viewID=table_viewID, model_shape=model_shape
            )
        )
        self.select_worker.registerProgress.connect(
            lambda register: self.__workerSelectOnRegisterProgress(
                register=register, table_viewID=table_viewID
            )
        )
        self.select_worker.finished.connect(
            lambda: self.__workerSelectOnFinished(table_viewID=table_viewID)
        )

        self.worker_manager.addTask(
            worker=self.select_worker, priority=WorkerPriority.MEDIUM
        )

        return None

    @Slot(object, tuple)
    def __workerSelectOnCountFinished(
        self, table_viewID: TableViewId, model_shape: tuple[int, int] = None
    ) -> None:
        """
        ...
        model_shape : tuple[int, int]
            ...
            acumulador
        """
        self.__setNpDataAccumulator(table_viewID=table_viewID, model_shape=model_shape)

        self.startProgressBar(table_viewID=table_viewID, max_val=model_shape[0])
        return None

    @Slot(tuple, object)
    def __workerSelectOnRegisterProgress(
        self, register: tuple[Any], table_viewID: TableViewId
    ) -> None:
        """
        ...
        match table_viewID.name:
            case "INVEN_TABLE_VIEW":
                ...
                self.updateProgressBar(table_viewID=table_viewID, value=register[0])
            ...
            case "SALES_TABLE_VIEW":
                ...
                try:
                    dt = datetime.strptime(
                        _date_time[-1], DateAndTimeFormat.DIR_DATETIME_ISO_8601.value
                    )
                    _date_time = list(_date_time)
                    _date_time[-1] = dt.strftime(
    ...
    @Slot(object, int)
    def updateProgressBar(self, table_viewID: TableViewId, value: int = None) -> None:
        """
    ...
    @Slot(str)
    def __workerSelectOnFinished(
        self, table_viewID: TableViewId, READ_OPERATION: bool = True
    ) -> None:
        """
    ...
    def handleTableCreateRow(self, table_viewID: TableViewId) -> None:
        ...
        match table_viewID.name:
            case "INVEN_TABLE_VIEW":
                productDialog = (
                    ProductDialog()
                )  # QDialog para añadir un producto nuevo a 'tv_inventory_data'
                productDialog.setAttribute(
                    Qt.WA_DeleteOnClose, True
                )  # destruye el dialog cuando se cierra
    ...
    @Slot(object, dict)
    def insertDataIntoModel(
        self, table_viewID: TableViewId, data_to_insert: dict[Any]
    ) -> None:
        """
    ...
    def __updateStockOnSaleCreation(self, data_to_insert: dict[Any]) -> None:
        ...
        if self.inventory_data_model.modelHasData():
            ...
            found_index_row = self.inventory_data_model.match(
                self.inventory_data_model.index(
                    0, InvViewCols.INV_PRODUCT_NAME.value
                ),  # busca desde la 1ra fila, 2da columna (de nombre).
            ...
            if found_index_row is not None:
                ...
                target_index = self.inventory_data_model.index(
                    found_index_row, InvViewCols.INV_STOCK.value
                )
                curr_stock, measurement_unit = target_index.data(
                ...
                try:
                    new_value = round(
                        float(curr_stock.replace(",", ".").strip())
                        - float(data_to_insert["product_quantity"]),
                        2,
                    )
                    new_value = f"{new_value} {measurement_unit}"

                    self.inventory_data_model.setData(
                        target_index, new_value, Qt.ItemDataRole.EditRole
                    )
    ...
    @Slot(object)
    def confirmedInventoryRowsDeletion(
        self, base_model_rows_selected: tuple[int]
    ) -> None:
        """
    ...
    def __accountsThatHaveBalance(
        self, selected_accounts: tuple[int]
    ) -> list[list[str, str]]:
        """
        ...
        for acc_row in selected_accounts:
            ...
            if str(_balance.data(Qt.ItemDataRole.DisplayRole)).strip("$ ") not in (
                "0",
                "0.0",
            ):
                _data.append(
    ...
    # generales
    def __getDeleteData(
        self, table_viewID: TableViewId, selected_rows: Iterable
    ) -> Iterable[tuple | dict]:
        """
        ...
        match table_viewID.name:
            ...
            case "SALES_TABLE_VIEW":
                ...
                with self._db_repo as db_repo:
                    ...
                    _placeholders = ",".join(
                        ["?"] * len(data)
                    )  # crea una cadena de '?,?,?...' largo como la cantidad de ids
    ...
    def deleteRowsFromDatabase(
        self, table_viewID: TableViewId, ids: Iterable, row_count: int
    ) -> None:
        """
    ...
    def __initDeleteWorker(
        ...
        match table_viewID:
            case TableViewId.INVEN_TABLE_VIEW:
                self.delete_worker = WorkerDelete(
                    table_viewID=table_viewID, del_sql=del_sql, del_params=del_params
                )

            case _:
                self.delete_worker = WorkerDelete(
                    table_viewID=table_viewID, mult_sql=del_sql, del_params=del_params
                )

        self.delete_worker.thread().started.connect(
            self.startProgressBar(table_viewID=table_viewID, max_val=row_count)
        )
        self.delete_worker.progress.connect(
            lambda n: self.updateProgressBar(table_viewID=table_viewID, value=n)
        )
        self.delete_worker.finished.connect(lambda: self.hideProgressBar(table_viewID))

        self.worker_manager.addTask(
            worker=self.delete_worker, priority=WorkerPriority.HIGH
        )
        return None
    ...
    def __onDelegateValidationFailed(self, feedback: tuple[TableViewId, str]) -> None:
        ...
        match feedback[0].name:
            case "INVEN_TABLE_VIEW":
                ...
                self.ui.label_feedbackInventory.setStyleSheet(
            ...
            case "SALES_TABLE_VIEW":
                ...
                self.ui.label_feedbackSales.setStyleSheet(
                    WidgetStyle.FIELD_INVALID_VAL.value
                )
            ...
            case "DEBTS_TABLE_VIEW":
                ...
                self.ui.label_feedbackDebts.setStyleSheet(
                    WidgetStyle.FIELD_INVALID_VAL.value
                )
    ...
    # ¡¡ ....... inventario ........................................................................
    @Slot(int, object)
    def onInventoryModelDataToUpdate(
        self, column: int, new_values: tuple[int, Any]
    ) -> None:
        """
        ...
        match column:
            ...
            case InvViewCols.INV_NORMAL_PRICE.value:
                ...
                self.__updateDebtsOnPriceChange(
                    price_type=InventoryPriceType.NORMAL, params=(IDproduct,)
                )
            ...
            case InvViewCols.INV_COMERCIAL_PRICE.value:
                ...
                # actualiza en Deudas
                self.__updateDebtsOnPriceChange(
                    price_type=InventoryPriceType.COMERCIAL, params=(IDproduct,)
                )
        return None

    def __updateDebtsOnPriceChange(
        self, price_type: InventoryPriceType, params: tuple[int] = None
    ) -> None:
        """
    ...
    def __onSalesModelDataToUpdate(
        self,
        column: int,
        IDsales_detail: int,
        new_val: Any,
        quantity_index: QModelIndex = None,
    ) -> None:
        """
                    ...
                    self.sales_data_model.updateMeasurementUnit(
                        ...
                        new_value=self._db_repo.selectRegisters(
                            data_sql="""SELECT COALESCE(unidad_medida, '') 
                                        ...
                                        WHERE nombre = ?;""",
                            data_params=(
                                self.sales_data_model._data[quantity_index.row(), 4],
                            ),
                        )[0][0],
                    )
    ...
    # ¡¡ ....... deudas ............................................................................
    @Slot(object)
    def __updateViewOnDebtsBalanceChanged(
        self, data: tuple[QModelIndex, float]
    ) -> None:
        """
    ...
    @Slot(int, int, object)
    def __onDebtsModelDataToUpdate(
        self, column: int, ID_debtor: int, new_val: Any
    ) -> None:
        """
    ...
    def handleCheckboxStateChange(self) -> None:
        ...
        """
        if (
            self.ui.checkbox_unit_prices.isChecked()
            or self.ui.checkbox_comercial_prices.isChecked()
        ):
    ...
    def __onPercentageValidatorFailed(self, error_message: str) -> None:
        ...
        self.ui.label_feedbackChangePercentage.setStyleSheet(
    ...
    def onLePercentageEditingFinished(self) -> None:
        ...
        self.__calculateNewPrices(
            percentage=percentage, selected_indexes=selected_indexes
        )
        return None
    ...
    def __getInventoryMappedIndexes(self, column: int) -> list[QModelIndex]:
        ...
        # mapea los índices
        return [
            proxy_model.mapToSource(proxy_model.index(row, column))
            for row in selected_rows
        ]

    def __calculateNewPrices(
        self, percentage: float, selected_indexes: list[QModelIndex]
    ) -> None:
        """
        ...
        if self.ui.checkbox_unit_prices.isChecked():
            for idx in selected_indexes:
                ...
                self.inventory_data_model.setData(index=idx, value=_new_val)
    ...
    def onPaidValidationSucceded(self) -> None:
        ...
        self.ui.lineEdit_paid.setStyleSheet(
            ...
            if self.ui.lineEdit_paid.text().strip()
            else ""
        )
    ...
    def addSalesInputListItem(self) -> None:
        ...
        item.deleteItem.connect(
            lambda list_item_name: self.onSalesItemDeletion(
                list_widget_item=list_widget_item, list_item_name=list_item_name
            )
        )
        item.fieldsValuesChanged.connect(
            lambda values: self.onSalesItemFieldsValuesChanged(
                values=values, object_name=object_name
            )
        )
    ...
    def onSalesItemDeletion(
        self, list_widget_item: QListWidgetItem, list_item_name: str
    ) -> None:
        """
    ...
    @Slot(object)
    def onSalesItemFieldsValuesChanged(
        self, values: dict[str, Any], object_name: str
    ) -> None:
        """
    ...
    def validateSalesItemsFields(self) -> None:
        ...
        try:
            ...
            if self.ui.sales_input_list.count() > 0 and all(
                [
                    item[SaleFields.IS_ALL_VALID.name]
            ...
            KeyError
        ):  # salta cuando no hay key 'IS_ALL_VALID', y pasa cuando se elimina
    ...
    def __onValidSalesItemsFields(self) -> None:
        ...
        subtotals: list[
            float
        ] = []  # lista con los subtotales de los items, es usado por 'completer_values' y
        ...
        subtotals = [
            subtotal[SaleFields.SUBTOTAL.name]
            for subtotal in self.DICT_ITEMS_VALUES.values()
        ]
        ...
        try:
            ...
            TypeError
        ):  # salta cuando en subtotals hay un subtotal 'None' (esto pasa porque
    ...
    def __createSalesCompleter(self, subtotals: list[float], total_cost: str) -> None:
        ...
        completer_values = [
            str(value).replace(".", ",") for value in subtotals + [total_cost]
        ]  # concatena 'subtotals' y
    ...
    def onFinishedSale(self) -> None:
        ...
        if total_paid < self.TOTAL_COST:
            ...
            dialog.debtorChosen.connect(
                lambda debtor_id: self.finishedSaleOnDebtorChosen(
                    debtor_id=debtor_id, total_paid=total_paid, dt_iso8601=_dt
                )
            )
    ...
    @Slot(tuple)
    def finishedSaleOnDebtorChosen(
        self, debtor_id: int, total_paid: float, dt_iso8601: datetime
    ) -> None:
        """
                ...
                if total_due < 0:  # el producto es deuda
                    db_repo.insertRegister(
                        ins_sql="""INSERT INTO Deudas(
                                        ...
                                        eliminado) 
                                    VALUES(?, ?, ?, 0);""",
                        ins_params=(dt_iso8601, abs(total_due), debtor_id),
                    )
    ...
    def setInitialDateRange(self) -> None:
        ...
        with QSignalBlocker(self.ui.dateEdit_from_date):
            ...
            self.ui.dateEdit_from_date.setMaximumDate(QDate.currentDate())
            self.ui.dateEdit_from_date.setDate(getWeekStartDate(QDate.currentDate()))

        with QSignalBlocker(self.ui.dateEdit_to_date):
            # ? ... y acá cambio temporalmente la fecha mínima por la mismo
            self.ui.dateEdit_to_date.setMinimumDate(QDate.currentDate())
            self.ui.dateEdit_to_date.setDate(QDate().currentDate())

        # vuelve a validar los rangos
        self.validateDateRange(
            dateedit=self.ui.dateEdit_from_date, date=self.ui.dateEdit_from_date.date()
        )
        self.validateDateRange(
            dateedit=self.ui.dateEdit_to_date, date=self.ui.dateEdit_to_date.date()
        )
        return None
...
def setup_spanish_translator(app:QApplication) -> None:
    """
    Instancia el traductor a español del programa.
    """
