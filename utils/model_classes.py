...
class InventoryTableModel(QAbstractTableModel):
    ...
    dataToUpdate: Signal = Signal(
        object
    )  # ? emite tuple(col:int, id:int, nuevo_valor:str).
        ...
        if role == Qt.ItemDataRole.EditRole:
            match index.column():
                ...
                case 3:  # stock (con unidad de medida)
                    ...
                    if (
                        self._data[index.row()][4] == full_stock[0]
                        and self._data[index.row()][5] == full_stock[1]
                    ):
                        return False
        ...
        match role:
            case Qt.ItemDataRole.DisplayRole:
                match col:
                    ...
                    case 3:  # stock
                        return str(
                            f"{self._data[row][4]} {self._data[row][5]}"
                        ).replace(".", ",")
...
class DebtsTableModel(QAbstractTableModel):
            ...
            match index.column():
                    ...
                    self.dataToUpdate.emit(
                        {
                            "column": index.column(),
                            "IDdebtor": self._data[
                                index.row(), DebtorModelCols.DEBTS_IDDEBTOR.value
                            ],
                            "new_value": value,
                        }
                    )
                ...
                case DebtorViewCols.DEBTS_PHONE_NUMBER.value:  # número de teléfono
                    if value == self.data(index):
                        return False
                    self._data[
                        index.row(), DebtorModelCols.DEBTS_PHONE_NUMBER.value
                    ] = value
                ...
                case DebtorViewCols.DEBTS_DIRECTION.value:  # dirección
                    if value == self.data(index):
                        return False
                    self._data[index.row(), DebtorModelCols.DEBTS_DIRECTION.value] = (
                        value
                    )
                ...
                case DebtorViewCols.DEBTS_POSTAL_CODE.value:  # código postal
                    if value == self.data(index):
                        return False
                    self._data[index.row(), DebtorModelCols.DEBTS_POSTAL_CODE.value] = (
                        value
                    )
                ...
                case DebtorViewCols.DEBTS_BALANCE.value:  # balance
                    if value == self.data(index):
                        return False
                    self._data[
                        index.row(), DebtorModelCols.DEBTS_TOTAL_BALANCE.value
                    ] = value
    ...
    def data(
        ...
        match role:
            case Qt.ItemDataRole.DisplayRole:
                match col:
                    ...
                    case DebtorViewCols.DEBTS_DIRECTION.value:
                        return (
                            f"{self._data[row, DebtorModelCols.DEBTS_DIRECTION.value]}"
                        )
            ...
            case Qt.ItemDataRole.BackgroundRole:
                match col:
                    case (
                        DebtorViewCols.DEBTS_PHONE_NUMBER.value
                    ...
                    case DebtorViewCols.DEBTS_BALANCE.value:
                        ...
                        if (
                            float(
                                self._data[
                                    row, DebtorModelCols.DEBTS_TOTAL_BALANCE.value
                                ]
                            )
                            < 0
                        ):
    ...
    def insertRows(
        ...
        dict_to_ndarray: ndarray = array(  # convierte el dict a un array de Numpy
            object=[
                data_to_insert[DebtorModelCols.DEBTS_IDDEBTOR.name],
...
class ProductsBalanceModel(QAbstractTableModel):
        ...
        if role == Qt.ItemDataRole.EditRole:
            match index.column():
                case DebtsViewCols.PRODS_BAL_DATETIME.value:
                    if value == self.data(index):
                        return False
                    self._data[index.row(), DebtsModelCols.PRODS_BAL_DATETIME.value] = (
                        value
                    )
                    ...
                    self.dataToUpdate.emit(
                        {
                            "column": index.column(),
                            "ID_sales_detail": self._data[
                                index.row(),
                                DebtsModelCols.PRODS_BAL_ID_SALES_DETAIL.value,
                            ],
                            "new_value": value,
                        }
                    )
                ...
                case DebtsViewCols.PRODS_BAL_DESCRIPTION.value:
                    if value == self.data(index):
                        return False
                    self._data[
                        index.row(), DebtsModelCols.PRODS_BAL_DESCRIPTION.value
                    ] = value
                ...
                case DebtsViewCols.PRODS_BAL_BALANCE.value:
                    if value == self.data(index):
                        return False
                    self._data[index.row(), DebtsModelCols.PRODS_BAL_BALANCE.value] = (
                        value
                    )
        ...
        match role:
            ...
            case Qt.ItemDataRole.BackgroundRole:
                if col == DebtsViewCols.PRODS_BAL_BALANCE.value:
                    # si el balance es negativo le da un fondo rojizo
                    if (
                        float(self._data[row, DebtsModelCols.PRODS_BAL_BALANCE.value])
                        < 0
                    ):
                        return TableBgColors.DEBTS_NEGATIVE_BALANCE.value
    ...
    def getBalance(self) -> float:
        ...
        if self.modelHasData():
            return round(
                sum(
                    list(self._data[:, DebtsModelCols.PRODS_BAL_BALANCE.value]), start=0
                ),
                ndigits=2,
            )
