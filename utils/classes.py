...
class ProductValues(QObject):
    ...
    validityChanged: Signal = (
        Signal()
    )  # se emite para activar/desactivar el botón 'Ok' del dialog
...
class ProductDialog(QDialog):
    ...
    dataFilled: Signal = Signal(
        object
    )  # emite un dict con todos los datos introducidos a MainWindow
    ...
    def setup_validators(self) -> None:
        ...
        self.productDialog_ui.lineedit_productDescription.setValidator(
            self.description_validator
        )
        self.productDialog_ui.lineedit_productStock.setValidator(self.stock_validator)
        self.productDialog_ui.lineedit_measurementUnit.setValidator(
            self.stock_unit_validator
        )
        self.productDialog_ui.lineedit_productUnitPrice.setValidator(
            self.unit_price_validator
        )
        self.productDialog_ui.lineedit_productComercialPrice.setValidator(
            self.comerc_price_validator
        )
    ...
    def setup_ui(self) -> None:
        ...
        self.productDialog_ui.buttonBox.button(QDialogButtonBox.Cancel).setText(
            "Cancelar"
        )
    ...
    def __setInitialStyles(self):
        ...
        """
        # flecha del combobox
        self.productDialog_ui.frame_productCategory.setStyleSheet(
            WidgetStyle.DEF_COMBOBOX_ARROW_ICON.value
        )

        self.productDialog_ui.buttonBox.setStyleSheet(
            WidgetStyle.DIALOG_CANCEL_BTN_STYLE.value
        )
        return None
    ...
    def setup_signals(self) -> None:
        ...
        self.name_validator.validationFailed.connect(
            lambda error_message: self.validatorOnValidationFailed(
                field_validated=ProductFields.NAME, error_message=error_message
            )
        )
        ...
        self.unit_price_validator.validationFailed.connect(
            lambda error_message: self.validatorOnValidationFailed(
                field_validated=ProductFields.NORMAL_PRICE, error_message=error_message
            )
        )
    ...
    def formatField(self, field_to_format: ProductFields) -> None:
        ...
        match field_to_format:
            ...
            case ProductFields.COMERCIAL_PRICE:
                text = (
                    self.productDialog_ui.lineedit_productComercialPrice.text().replace(
                        ".", ","
                    )
                )
                if text.endswith(","):
    ...
    @Slot(object, str)
    def validatorOnValidationFailed(
        self, field_validated: ProductFields, error_message: str
    ) -> None:
        """
    ...
    def toggleOkButton(self) -> None:
        ...
        if self.product_values.isAllValid():
            ...
            self.productDialog_ui.buttonBox.button(QDialogButtonBox.Ok).setEnabled(
                False
            )
        return None
            ...
            self.dataFilled.emit(
                {
                    ...
                    "product_comercial_price": float(
                        self.product_values.getComercialPrice()
                    ),
...
class CategoryDescDialog(QDialog):
    ...
    **QDialog** que permite modificar la descripción de la categoría
    ...
    def setup_editor(self) -> None:
        ...
        _text = self.categ_desc_dialog.te_category_desc.document().toPlainText()
        self.categ_desc_dialog.label_show_character_count.setText(
            self.categ_desc_dialog.label_show_character_count.text().replace(
                "x", f"{len(_text)}"
            )
        )
            ...
            case QValidator.State.Invalid:
                ...
                with QSignalBlocker(self.categ_desc_dialog.te_category_desc):
                    self.categ_desc_dialog.te_category_desc.setPlainText(
                        ...
                        else text[: self.MAX_TEXT_LENGTH]
                    )
...
class SaleValues(QObject):
    ...
    priceCheckboxEnabled: Signal = Signal(
        bool
    )  # si se elige un producto que no tiene precio comercial
    ...
    costIsDiffThanPaid: Signal = Signal(
        bool
    )  # se emite para mostrar/esconder el frame de deudor en el dialog
    ...
    def __init__(self) -> None:
        ...
        # cuenta corriente
        self.__THERE_IS_DEBT: bool = (
            False  # flag que determina si en el dialog 'debtor_data' está
        )
    ...
    def setup_signals(self) -> None:
        ...
        self.totalCostChanged.connect(lambda: self.setSaleDetail(sale_detail=None))
    ...
    def setSaleDetail(self, sale_detail: str = None) -> None:
        ...
        match sale_detail:
            case None:  # generado programáticamente
                ...
                _product_name = (
                    _product_name if not self.__product_name else self.__product_name
                )
                self.__sale_detail = f"{_quantity} de {_product_name} ({price_type})"

            case _:  # lo escribió el usuario
                _pattern = compile(
                    Regex.SALES_DETAILS_PRICE_TYPE.value, flags=IGNORECASE
                )
                new_sale_detail = sub(pattern=_pattern, repl="", string=sale_detail)
    ...
    def __comercialPriceExists(self, product_name: str) -> bool:
        ...
        comerc_price: (
            float | None | str
        )  # si None o str es porque no tiene precio comercial
    ...
    def setDatetime(self, datetime: QDateTime) -> None:
        ...
        """
        self.__datetime = datetime.toString(
    ...
    def getPriceType(self) -> InventoryPriceType:
        return (
            InventoryPriceType.NORMAL
            if not self.__is_comercial_price
            else InventoryPriceType.COMERCIAL
        )
    ...
    def isProductNameValid(self) -> bool:
        ...
        """
        return (
            False if not self.__FIELDS_VALIDITY[SaleFields.PRODUCT_NAME.name] else True
        )
    ...
    def isAllValid(self) -> bool:
        ...
        pos_to_check = (
            pos_to_check if self.thereIsDebt() else pos_to_check - 2
        )  # si no hay deuda no tiene
...
class SaleDialog(QDialog):
    ...
    minHeightChanged: Signal = Signal(
        int
    )  # emite el nuevo minimumHeight, sirve para activar/desactivar el botón Ok.
    dataFilled: Signal = Signal(
        object
    )  # emite un dict con todos los datos introducidos a MainWindow.
    ...
    def __setInitialStyles(self):
        ...
        # flecha del combobox
        self.saleDialog_ui.sale_data.setStyleSheet(
            WidgetStyle.DEF_COMBOBOX_ARROW_ICON.value
        )
        self.saleDialog_ui.debtor_data.setStyleSheet(
            WidgetStyle.DEF_COMBOBOX_ARROW_ICON.value
        )
    ...
    def setup_validators(self) -> None:
        ...
        self.sale_detail_validator = SaleDetailsValidator(
            self.saleDialog_ui.lineEdit_saleDetail
        )
        self.quantity_validator = SaleQuantityValidator(
            self.saleDialog_ui.lineEdit_productQuantity
        )
        self.total_paid_validator = SalePaidValidator(
            self.saleDialog_ui.lineEdit_totalPaid
        )

        self.saleDialog_ui.lineEdit_saleDetail.setValidator(self.sale_detail_validator)
        self.saleDialog_ui.lineEdit_productQuantity.setValidator(
    ...
    def setup_signals(self) -> None:
        ...
        self.sale_values.saleDetailChanged.connect(
            lambda sale_detail: self.saleDialog_ui.lineEdit_saleDetail.setText(
                sale_detail
            )
        )
        ...
        self.quantity_validator.validationFailed.connect(
            lambda error_message: self.validatorOnValidationFailed(
                field_validated=SaleFields.QUANTITY, error_message=error_message
            )
        )
        ...
        self.total_paid_validator.validationFailed.connect(
            lambda error_message: self.validatorOnValidationFailed(
                field_validated=SaleFields.TOTAL_PAID, error_message=error_message
            )
        )

        # datetimeedit
        self.saleDialog_ui.dateTimeEdit.dateTimeChanged.connect(
            self.sale_values.setDatetime
        )
        ...
        self.saleDialog_ui.cb_debtor_name.currentTextChanged.connect(
            self.onDebtorNameTextChanged
        )

        # combobox apellido (cuenta corriente)
        self.saleDialog_ui.cb_debtor_surname.currentIndexChanged.connect(
            self.onDebtorSurnameIndexChanged
        )
    ...
    def validatorOnValidationSucceded(self, field_validated: SaleFields) -> None:
        ...
        match field_validated:
            case SaleFields.QUANTITY:
                ...
                self.saleDialog_ui.lineEdit_productQuantity.setStyleSheet(
                    WidgetStyle.FIELD_VALID_VAL.value
                )
                self.saleDialog_ui.label_productQuantity_feedback.hide()
            ...
            case SaleFields.TOTAL_PAID:
                ...
                self.sale_values.setTotalPaid(
                    self.saleDialog_ui.lineEdit_totalPaid.text().replace(",", ".")
                )

                self.saleDialog_ui.lineEdit_totalPaid.setStyleSheet(
                    WidgetStyle.FIELD_VALID_VAL.value
                )
                self.saleDialog_ui.label_totalPaid_feedback.hide()
        return None
    ...
    @Slot(str, str)
    def validatorOnValidationFailed(
        self, field_validated: SaleFields, error_message: str
    ) -> None:
        """
    ...
    def onDebtorNameTextChanged(self, current_name: str) -> None:
        ...
        if current_name in self.DEBTORS_FULL_NAMES:
            ...
            self.sale_values.setFieldValidity(
                field=SaleFields.DEBTOR_NAME, validity=True
            )

        else:
            self.sale_values.setDebtorName(debtor_name="")
            self.sale_values.setFieldValidity(
                field=SaleFields.DEBTOR_NAME, validity=False
            )
        return None
    ...
    def onDebtorSurnameIndexChanged(self, index: int) -> None:
        ...
        if index != -1:
            ...
            self.sale_values.setFieldValidity(
                field=SaleFields.DEBTOR_SURNAME, validity=True
            )

        else:
            self.sale_values.setDebtorSurname(debtor_surname="")
            self.sale_values.setFieldValidity(
                field=SaleFields.DEBTOR_SURNAME, validity=False
            )
        return None
    ...
    def __setSaleDialogSize(
        self, min_width: int, min_height: int, hide_debtor_data: bool
    ) -> None:
        """
            ...
            values_to_dict = {
                ...
                "product_measurement_unit": db_repo.selectRegisters(
                    data_sql="""SELECT 
                                ...
                                FROM Productos 
                                WHERE nombre = ?;""",
                    data_params=(self.sale_values.getProductName(),),
                )[0][0],
...
class ListItemValues(QObject):
    ...
    priceTypeChanged: Signal = Signal(
        object
    )  # emite el nuevo tipo de precio como 'InventoryPriceType'.
    subtotalChanged: Signal = Signal(
        object
    )  # emite el nuevo subtotal, que puede ser float
    ...
    def __init__(self, object_name: str):
        ...
        self.__FIELDS_VALIDITY: dict[
            str, bool | None
        ] = {  # ? guarda valores True | False | None, siendo el valor de verdad
        ...
        self.subtotalChanged.connect(
            lambda: self.setSaleDetails(curr_sale_detail=self.__sale_details)
        )
    ...
    def setPriceType(
        self, price_type: InventoryPriceType = InventoryPriceType.NORMAL
    ) -> None:
        """
        ...
        """
        self.__is_comercial_price = (
            False if price_type == InventoryPriceType.NORMAL else True
        )
    ...
    def setSaleDetails(self, curr_sale_detail: str) -> None:
        ...
        pattern: Pattern = compile(
            pattern=Regex.SALES_DETAILS_FULL.value, flags=IGNORECASE
        )
        new_text: str
        ...
        if self.isAllValid():
            ...
            if fullmatch(pattern, curr_sale_detail) or curr_sale_detail.strip() == "":
                self.__sale_details = (
                    f"{self.getQuantity()} de {self.getProductName()} ({price_type})"
                )
...
class ListItemWidget(QWidget):
    ...
    def __init__(self, obj_name: str):
        ...
        self.field_values: ListItemValues = ListItemValues(
    ...
    def setup_ui(self, obj_name: str) -> None:
        ...
        self.listItem.comboBox_productName.setInsertPolicy(
    ...
    def setup_validators(self) -> None:
        self.quantity_validator = SaleQuantityValidator(
            self.listItem.lineEdit_productQuantity
        )
        self.sale_detail_validator = SaleDetailsValidator(
            self.listItem.lineEdit_saleDetail
        )
    ...
    def setup_signals(self) -> None:
        ...
        # --- SEÑALES --------------------------------------------------
        # nombre de producto
        self.listItem.comboBox_productName.currentIndexChanged.connect(
            self.onComboboxIndexChange
        )

        # cantidad de producto
        self.listItem.lineEdit_productQuantity.editingFinished.connect(
            self.onQuantityEditingFinished
        )

        self.quantity_validator.validationSucceeded.connect(
            self.validatorOnValidationSucceded
        )
        self.quantity_validator.validationFailed.connect(
            self.validatorOnValidationFailed
        )

        # checkbox tipo de precio
        self.listItem.checkBox_comercialPrice.checkStateChanged.connect(
            self.onCheckStateChange
        )

        # cambios en el detalle de venta
        self.listItem.lineEdit_saleDetail.editingFinished.connect(
            lambda: self.onSaleDetailEdit(text=self.listItem.lineEdit_saleDetail.text())
        )
    ...
    def validatorOnValidationSucceded(self) -> None:
        ...
        if self.field_values.isNameValid():
            ...
            self.listItem.label_quantityFeedback.setText(
                f"stock disponible: {_avail_stock[0]} {_avail_stock[1]}"
            )
    ...
    def __onValidProductName(self, curr_name: str) -> None:
        ...
        self.quantity_validator.validate(
            self.listItem.lineEdit_productQuantity.text(), 0
        )
    ...
    def __showAvailableStock(self) -> None:
        ...
        self.listItem.label_quantityFeedback.setText(
            "El stock disponible es de {} {}".format(
                self.quantity_validator.AVAILABLE_STOCK[0],
                self.quantity_validator.AVAILABLE_STOCK[1],
            )
        )
        return None
    ...
    def onQuantityEditingFinished(self) -> None:
        ...
        # actualiza 'self.field_values'
        self.field_values.setQuantity(float(field_text.replace(",", ".")))
...
class DebtorDataDialog(QDialog):
    ...
    def setup_ui(self) -> None:
        ...
        self.debtorData.buttonBox.setStyleSheet(
    ...
    def setup_variables(self) -> None:
        ...
        # validadores
        self.debtor_name_validator = DebtorNameValidator(
            self.debtorData.lineEdit_debtorName
        )
        self.debtor_surname_validator = DebtorSurnameValidator(
            self.debtorData.lineEdit_debtorSurname
        )
        self.phone_number_validator = DebtorPhoneNumberValidator(
            self.debtorData.lineEdit_phoneNumber
        )
        self.direction_validator = DebtorDirectionValidator(
            self.debtorData.lineEdit_direction
        )
        self.postal_code_validator = DebtorPostalCodeValidator(
            self.debtorData.lineEdit_postalCode
        )

        self.debtorData.lineEdit_debtorName.setValidator(self.debtor_name_validator)
        self.debtorData.lineEdit_debtorSurname.setValidator(
            self.debtor_surname_validator
        )
    ...
    def setup_signals(self) -> None:
        ...
        self.debtor_name_validator.validationFailed.connect(
            lambda error_message: self.validatorOnValidationFailed(
                field_validated=DebtsFields.NAME, error_message=error_message
            )
        )
        ...
        self.debtor_values.nameChanged.connect(
            lambda new_val: self.__updateFieldValue(
                field_to_update=DebtsFields.NAME, new_val=new_val
            )
        )
        ...
        self.debtor_surname_validator.validationFailed.connect(
            lambda error_message: self.validatorOnValidationFailed(
                field_validated=DebtsFields.SURNAME, error_message=error_message
            )
        )
        ...
        self.debtor_values.surnameChanged.connect(
            lambda new_val: self.__updateFieldValue(
                field_to_update=DebtsFields.SURNAME, new_val=new_val
            )
        )
        ...
        self.phone_number_validator.validationFailed.connect(
            lambda error_message: self.validatorOnValidationFailed(
                field_validated=DebtsFields.PHONE_NUMB, error_message=error_message
            )
        )
        ...
        self.debtor_values.phoneChanged.connect(
            lambda new_val: self.__updateFieldValue(
                field_to_update=DebtsFields.PHONE_NUMB, new_val=new_val
            )
        )
        ...
        self.direction_validator.validationFailed.connect(
            lambda error_message: self.validatorOnValidationFailed(
                field_validated=DebtsFields.DIRECTION, error_message=error_message
            )
        )
        ...
        self.debtor_values.directionChanged.connect(
            lambda new_val: self.__updateFieldValue(
                field_to_update=DebtsFields.DIRECTION, new_val=new_val
            )
        )
        ...
        self.postal_code_validator.validationFailed.connect(
            lambda error_message: self.validatorOnValidationFailed(
                field_validated=DebtsFields.POSTAL_CODE, error_message=error_message
            )
        )
        ...
        self.debtor_values.postalCodeChanged.connect(
            lambda new_val: self.__updateFieldValue(
                field_to_update=DebtsFields.POSTAL_CODE, new_val=new_val
            )
        )
    ...
    def validatorOnValidationSucceded(self, field_validated: DebtsFields) -> None:
        ...
        self.debtor_values.setFieldValidity(field=field_validated, validity=True)
        ...
        match field_validated:
            ...
            case DebtsFields.PHONE_NUMB:
                ...
                if (
                    self.debtorData.lineEdit_phoneNumber.isEnabled()
            ...
            case DebtsFields.POSTAL_CODE:
                ...
                # ? por la misma razón
                if (
                    self.debtorData.lineEdit_postalCode.isEnabled()
                    and self.debtorData.lineEdit_postalCode.text()
                ):
    ...
    @Slot(object, str)
    def validatorOnValidationFailed(
        self, field_validated: DebtsFields, error_message: str
    ) -> None:
        """
        Cambia el valor del flag asociado al campo que fue validado 'field_validated'
        ...
        """
        self.debtor_values.setFieldValidity(field=field_validated, validity=False)
    ...
    def formatField(self, field_to_format: DebtsFields) -> None:
        ...
        field_text: str
        phone_number: (
            PhoneNumber  # se usa cuando el campo a formatear es el de núm. de teléfono
        )
...
class ProductsBalanceDialog(QDialog):
    ...
    def __init__(
        self, debtor_id: int, curr_balance: float, table_view: QTableView
    ) -> None:
    ...
    def setup_ui(self) -> None:
        ...
        self.products_balance_dialog.central_widget.setGraphicsEffect(
            self.shadow_effect
        )
        ...
        self.event_filter = BackgroundEventFilter(
            self.products_balance_dialog.tv_balance_products
        )
        self.products_balance_dialog.tv_balance_products.viewport().installEventFilter(
            self.event_filter
        )
        return None
    ...
    def setup_validators(self) -> None:
        ...
        self.products_balance_dialog.search_bar.setValidator(self.search_bar_validator)
    ...
    def setup_model(self) -> None:
        ...
        self.products_balance_proxy_model.setFilterCaseSensitivity(
            Qt.CaseSensitivity.CaseInsensitive
        )
        self.products_balance_proxy_model.setSourceModel(self.products_balance_model)

        # table view
        self.products_balance_dialog.tv_balance_products.setModel(
            self.products_balance_proxy_model
        )
        self.products_balance_dialog.tv_balance_products.setSortingEnabled(True)

        # selection model
        self.tv_selection_model = (
            self.products_balance_dialog.tv_balance_products.selectionModel()
        )
    ...
    def setup_signals(self) -> None:
        ...
        self.tv_selection_model.selectionChanged.connect(self.toggleLeReduceDebt)
        self.tv_selection_model.selectionChanged.connect(self.toggleDeleteDebtButton)

        # eliminación de deudas
        self.products_balance_dialog.btn_delete_debt.clicked.connect(self.onDeleteDebts)
            ...
            for i, reg in enumerate(data):
                ...
                try:
                    dt = datetime.strptime(
                        _date_time, DateAndTimeFormat.DIR_DATETIME_ISO_8601.value
                    )
                    _date_time = dt.strftime(
    ...
    def __onProductsBalanceBaseModelRowsSelected(
        ...
        for row in base_model_rows_selected:
            ids_to_delete.append(self.products_balance_proxy_model.getSaleDetailID(row))
    ...
    @Slot(int, int, object)
    def updateValuesInDatabase(
        self, column: int, IDsales_detail: int, new_val: Any
    ) -> None:
        """
        ...
        with DatabaseRepository() as db_repo:
            match column:
                case DebtsViewCols.PRODS_BAL_DATETIME.value:
                    ...
                    _datetime_to_iso = (
                        new_val if not _datetime_to_iso else _datetime_to_iso
                    )
    ...
    def onLEReduceDebtReturnPressed(self) -> None:
        ...
        for idx in _mapped_indexes:
            ...
            if value >= _idx_balance:
                value -= _idx_balance
                value = round(value, 2)
                self.products_balance_model.setData(index=idx, value=0.0)

            # si el valor es menor descuenta el valor y deja de iterar
            else:
                self.products_balance_model.setData(
                    index=idx, value=round(_idx_balance - value, 2)
                )
    ...
    def __getSelectedMappedIndexes(self) -> list[QModelIndex]:
        ...
        # mapea los índices
        return [
            proxy_model.mapToSource(proxy_model.index(row, col))
    ...
    def showEvent(self, event: QShowEvent):
        ...
        table_bottom_right: QPoint = self.__table_view.mapToGlobal(
            table_rect.bottomRight()
        )
